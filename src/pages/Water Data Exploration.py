...
def main():
    ...
    options = [raw_water_data.iloc[0]["date"].to_pydatetime(), raw_water_data.iloc[-1]["date"].to_pydatetime()]
    ...
    with fit_col:
        with st.expander("Fit model to data", expanded=True):
            train_from = pd.to_datetime(st.date_input("Training Start Date", options[0], min_value=options[0], max_value=options[1]))
            train_to = pd.to_datetime(
                st.date_input("Training End Date", options[1], min_value=options[0], max_value=options[1])
            )
            ...
            # del slider_from
            # del slider_to
            fit_type = st.selectbox("Model Type", ("None", "Power Law", "Polynomial Model"))

            df = raw_water_data
            discharge_by_stage_water_data = df[(df["date"] >= train_from) & (df["date"] <= train_to)]
            ...
            if fit_type == "Power Law":
                model_fit = fit_power_law(train_stage, train_discharge, train_from, train_to)
            elif fit_type == "Polynomial Model":
                degree = st.number_input("fit-degree", min_value=2, max_value=5, value=2)
                model_fit = fit_polynomial(train_stage, train_discharge, degree, train_from, train_to)
        ...
        if st.checkbox("View rating table for fit model"):
            with st.expander("Rating Table", expanded=True):
                if not model_fit:
                    ...
                    min_stage = float(raw_water_data["stage_val"].min() // 5 * 5)  # nearest lower 5
                    min_val = round(raw_water_data["stage_val"].min(), 1)
                    max_stage = float(raw_water_data["stage_val"].max() // 5 * 5 + 5)  # nearest upper 5
                    columns = iter(st.columns(2))
                    with next(columns):
                        start = st.number_input(
                            "Start Stage Level", min_value=min_stage - 25, max_value=max_stage + 25, value=min_val,
                            step=0.1
                        )
                    with next(columns):
                    ...
                    st.dataframe(rating_table.style.format(formatter="{:.2f}"), hide_index=True)
    ...
    with data_col:
        with st.expander("Daily Stage and Discharge", expanded=True):
            ...
            df = raw_water_data
            daily_water_data = df[(df["date"] >= filter_from) & (df["date"] <= filter_to)]

            view_type = st.selectbox("View Option", ("Adjusted", "Raw", "Normalized"), label_visibility="collapsed")
            ...
            daily_fig.add_trace(
                go.Bar(x=daily_water_data["date"], y=daily_water_data[stage_key], name="Stage", marker_color="blue")
            )
            ...
            if model_fit:
                predicted_discharge = model_fit.predict(daily_water_data["stage_val"], normalized=False)
                daily_display_rmse = round(calculate_rmse(daily_water_data["discharge_rate"], predicted_discharge), 4)

                if normalize_data:
                    display_discharge = model_fit.predict(daily_water_data["stage_val"], normalized=True)
                else:
        ...
        with st.expander("Discharge by Stage Level", expanded=True):
            ...
            slider_from, slider_to = st.slider("Dates", options[0], options[1], value=options, key="dates-2")
            filter_from = pd.to_datetime(slider_from)
            filter_to = pd.to_datetime(slider_to)
            del slider_from
            del slider_to
            ...
            df = raw_water_data
            discharge_by_stage_water_data = df[(df["date"] >= filter_from) & (df["date"] <= filter_to)]
            discharge_by_stage_stage = discharge_by_stage_water_data["stage_val"].values
            discharge_by_stage_discharge = discharge_by_stage_water_data["discharge_rate"].values
            discharge_fig.add_trace(
                go.Scatter(x=discharge_by_stage_stage, y=discharge_by_stage_discharge, mode="markers", name="Data")
            )
            ...
            if model_fit is not None:
                ...
                predicted_discharge = model_fit.predict(discharge_by_stage_water_data["stage_val"], normalized=False)
                by_stage_rmse = round(calculate_rmse(discharge_by_stage_water_data["discharge_rate"], predicted_discharge), 4)
...
@st.cache_data
def fit_polynomial(stage, discharge, fit_degree: int, training_start: datetime, training_end: datetime):
